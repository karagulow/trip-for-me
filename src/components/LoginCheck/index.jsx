...
export const LoginCheck = ({ setLoginCheckOpen, organizerAuth, setIsAuth }) => {
  const loginUrl = organizerAuth ? '/lk/guide/id' : '/lk/tourist/id';
  ...
  return (
    <div className={styles.login_check}>
      <div className={styles.login_checkBlock}>
        <div
          className={styles.login_checkBlock__close}
          onClick={() => {
            setLoginCheckOpen(false);
        ...
        <h2 className={styles.login_checkBlock__title}>
          Введите одноразовый код
        </h2>
        <form className={styles.login_checkBlock__form}>
          <IMaskInput
            type="text"
            id="inputCode"
            value={inputValue}
        ...
        <p className={styles.login_checkBlock__text}>
          Сообщение с кодом проверки отправлено на ваше устройство. Повторный
          код можно получить через: <span>0:59</span>
        </p>
        <button className={styles.login_checkBlock__btn}>
